...
/**************************************************************************/
/**************************************************************************/
SPRESULT spell_animate_dead (int sn, int level, char_data *ch, void *vo, int target) {
      ...
      if ( obj == NULL ) {
                ch->println("Eh... you have to reanimate some corpse.");
            return NO_MANA;
        }
      ...
      {
                       ch->println("Eh... that's not a corpse.");
                  return NO_MANA;
      }
        ...
        if (obj->level > (ch->level + 5)) {
                ch->println("The spirit was too great for you to call.");
                return HALF_MANA;
        }

        if (ch->pet != NULL) {
                ch->println("You already have a follower.");
                return FULL_MANA;
      }
      
      
        MOB_INDEX_DATA *mIndex;
        mIndex = get_mob_index(MOB_VNUM_ZOMBIE);
        if (mIndex == NULL) {
                wiznet("*** BUG: get_mob_index returning NULL.",NULL,NULL,WIZ_SECURE,0,92);
                return NO_MANA;
        }
        mob = create_mobile(get_mob_index(500), ch->level);
      ...
      mob ->level = ch->level/2;
        mob->max_hit = mob->level * 5 + number_range(mob->level / 8, mob->level/10);
        mob->max_hit = mob->max_hit * 2;
        mob->hit = mob->max_hit;
        mob->max_mana = dice(mob->level,10);
      ...
      for (i = 0; i < 3; i++) {
        ...
        for (i = 0; i < MAX_STATS; i++) {
                mob->perm_stats[i] = 11 + mob->level/4;
        }
