...
void saymote( language_data *language, char_data *ch, char *argument, int sayflags)
{
   ...
   if(IS_SET(sayflags, SAYFLAG_IC))
    ...
    for( char_data *to= ch->in_room->people; to ; to = to->next_in_room )
    {
      ...
      // If we are not in character, and the recipient does not want to get OOC says, skip them
      if(!ic && HAS_CONFIG2(to, CONFIG2_NO_SAY_OOC) && !IS_OOC(ch))
         continue; 
		sprintf(format_buffer,"%s%s%s %s",
			(IS_IMMORTAL(to)?wizitext:""),			// [Wizi %d]
         IS_OOC(ch)||!ic?"`B(OOC) `x":"`G(IC) `x",         // (OOC) or (IC) 
	...
	{	// format what is sent to the actual speaker
		...
		if(IS_OOC(ch)){
         ...
         if(GAMESETTING3(GAMESET3_LANGUAGE_NAME_NOT_IN_SAYS)
            || language == language_unknown)
         {
            sprintf(format_buffer,"`x%s%sYou %s",
               (IS_IMMORTAL(ch)?wizitext:""), !ic?"`B(OOC) `x":"`G(IC) `x", tochar_sayask_text);               
         }else{
            sprintf(format_buffer,"`x%s%sYou %s in %s",
               (IS_IMMORTAL(ch)?wizitext:""), !ic?"`B(OOC) `x":"`G(IC) `x", tochar_sayask_text, language->name);
         }
      } 
