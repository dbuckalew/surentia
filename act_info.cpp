...
/**************************************************************************/
void do_autoallsayic(char_data *ch, char *)
{
    if(HAS_CONFIG2(ch, CONFIG2_ALL_SAY_IC))
    {
      ch->println( "Your say command will now be OOC, use isay for IC." );
      REMOVE_CONFIG2(ch, CONFIG2_ALL_SAY_IC);
    }else{
      ch->println( "Your say command will always be IC, you will not be able to make OOC says.");
      SET_CONFIG2(ch, CONFIG2_ALL_SAY_IC);
    }
   ch->titlebar("ABOUT AUTOALLSAYIC");
   ch->wrapln("Says now are flagged as `GIC`x or `BOOC`x.  Both you and the recipient of the say"
"will see the `GIC`x or `BOOC`x before the message.  You can determine if your say is"
"`GIC`x or `BOOC`x in several ways:`+"
"`+"
"If you are in an `BOOC`x room, all says will be `BOOC`x.  You cannot change it."
"However, if you are not in an `BOOC`x room, you have a choice of saying and "
"receiving `BOOC`x and `GIC`x says.  If you use the `Yisay`x (or `Y^`x) command, your says"
"will be `GIC`x.`+"
"`+"
"If you use `Ysay`x (or `Y'`x) command, the default will be for `BOOC`x says.  However,"
"setting `Yautoallsayic`x will make all of your says (or `Y'`x) become `GIC`x by default."
"You will have to toggle this flag off (type `Yautoallsayic`x again) to make"
"your says revert to `BOOC`x.`+"
"`+"
"The `Ysayto`x command is `BOOC`x for now, but this may change int he near future." 
"If you want to say something to a person and have it be in `GIC`x, type `Yisay >`x"
"`Y(character) (text)`x, but without the `Y()`x. You do need the `Y>`x however.`+"
"`+"
"If you do not want to receive `BOOC`x says, then type `Yautoblockoocsay`x and you" 
"will not receive `BOOC`x says except in `BOOC`x rooms. ");
   ch->titlebar("");
}
/**************************************************************************/
void do_autoblockoocsay(char_data *ch, char *)
{
    if(HAS_CONFIG2(ch, CONFIG2_NO_SAY_OOC))
    {
      ch->println( "You will now hear OOC says." );
      REMOVE_CONFIG2(ch, CONFIG2_NO_SAY_OOC);
    }else{
      ch->println( "You will now only hear IC says.");
      SET_CONFIG2(ch, CONFIG2_NO_SAY_OOC);
    }
   ch->titlebar("ABOUT AUTOBLOCKOOCSAY");
   ch->wrapln("Says now are flagged as `GIC`x or `BOOC`x.  Both you and the recipient of the say"
"will see the `GIC`x or `BOOC`x before the message.  You can determine if your say is"
"`GIC`x or `BOOC`x in several ways:`+"
"`+"
...
void do_autolist(char_data *ch, char *)
{
  ...
  ch->printlnf( "autoallsayic  %s", HAS_CONFIG2(ch, CONFIG2_ALL_SAY_IC)?alon:aloff);      
   ch->printlnf( "autoblockoocsay  %s", HAS_CONFIG2(ch, CONFIG2_NO_SAY_OOC)?alon:aloff); 
	if(IS_IMMORTAL(ch)){
